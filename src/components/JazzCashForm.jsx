  ...
  return (
    <>
      ...
      <div className="bg-white shadow-lg rounded-lg p-6 space-y-6 md:space-y-0 md:flex md:gap-6">
        {[1, 2, 3].map((num, idx) => (
          <div key={num} className="flex flex-col items-center gap-2 md:flex-1">
            <p className="w-8 h-8 flex items-center justify-center bg-primary-500 text-white rounded-full font-bold">
              {num}
            </p>
            <p className="text-gray-700 text-center">
              {idx === 0 &&
                "Pay your bill on our account, details above mentioned."}
              {idx === 1 &&
                "Upload the screen shot of payment proof enter your transaction ID and account name in the form fields below."}
              {idx === 2 && (
                <>
                  Click the <span className="font-semibold">"Pay Now"</span>{" "}
                  button to submit your payment details for verification.
                </>
              )}
            </p>
          </div>
      ...
      <div className="rounded-lg p-6 transition flex items-center gap-4 border-2 border-red-900">
        ...
        <div className="w-[30%]">
            <img src="/jazzcash-qrcode.jpg" alt="QR Code" className="h-full w-full" />
        </div>
